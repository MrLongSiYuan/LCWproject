<style lang="scss">
    ...
    .detail-page-main-left-item-content-tip{
        position: absolute;
        left: 10px;
        top: 0;
        padding: 6px 0 10px;
...
<template>
    <div>
        ...
        <div v-if="blockItem.isEdit" class="detail-block-box">
            <div class="detail-page-main-left-item-content-editbtn">
                <i class="icon iconfont_daydao_common" @click="editRowData">&#xe69c;</i>
                <i class="icon iconfont_daydao_common" @click="deleteRowData">&#xe6ae;</i>
            </div>
            <div class="detail-page-main-left-item-content-tip" v-for="item in blockItem.data" v-if="item.name == 'is_show' && item.value == '1'">展示</div>
            <ul class="detail-page-main-left-list">
                <li v-for="(showItem, itemIndex) in blockItem.data" :key="itemIndex" v-if="showItem.name == 'is_show' && showItem.cellType=='checkbox' ? false : true" :style="(showItem.name == 'is_show' && showItem.cellType=='checkbox') || showItem.cellType=='textarea' ? 'width: 100%;' : ''">
                    ...
                    <dl v-if="!(showItem.name == 'is_show' && showItem.cellType=='checkbox')">
                        <span class="detail-page-main-left-list-content"
                              v-if="showItem.cellType!='switch'"
                              :style="showItem.cellType == 'textarea' ? 'white-space: normal; width: calc(((100% + 135px) - 135px) - ((((100% + 135px) / 2) - 135px) * (1 / 3)));' : ''"
                              :title="showItem.value">{{showItem.value}}</span>
                        <span class="detail-page-main-left-list-content" v-if="showItem.cellType=='switch'" :title="showItem.value">{{showItem.value ? '是' : '否'}}</span>
                    </dl>
                </li>
            </ul>
        </div>
        ...
        <div v-if="!blockItem.isEdit" class="detail-edit-box detail-add-box">
            <Form ref="formCustom" :model="formCustom" :rules="ruleCustom">
                ...
                <FormItem v-for="(item, itemIndex) in blockItem.data" :key="itemIndex"
                    ...
                    <!-- 是否显示在列表 -->
                    <Checkbox v-model="item.value" :disabled="actionFlag">{{item.title}}</Checkbox>
                </FormItem>
    ...
    export default {
        ...
        methods: {
                ...
                try {
                    ...
                    _this.formatFormAndRuleCustom(function() {
                        ...
                        _this.$refs[name].validate((valid) => {
                            if (!valid) return _this.actionFlag = false; //console.log('表单验证失败!')
            ...
            formatFormAndRuleCustom(endFn) {
                ...
                _this.blockItem.data.forEach((item)=>{
                    if((item.cellType=='treeSelect' || item.cellType=='treeAdviceSelect')&& !item.id){
                        formatTreeSelectDatas(item,_this.treeSelecArray[item.keyValueBean.infoSetId]);
                    }
                    let key = item.name,
                        val = item.value;
                ...
                function formatTreeSelectDatas(searchItem,treeData){
                    treeData.forEach(function(item,index){
                        if(item.name==searchItem.value){
                            searchItem.id=item.id
                            return
                        }
                        if(item.children && item.children.length>0){
                            formatTreeSelectDatas(searchItem,item.children);
                        }
                    });
                }
            },
