...
<template>
    <!-- 关键词 -->
    <div>
        <div  :class="['input-control', {'pull-left':item.cellTypeId==='originTree'}]"  v-for="(item , index) in cloneData" v-if="item.isDefault!='1'">
            ...
            <!-- 组织树 -->
            <orgTree v-else-if="item.cellTypeId==='originTree'" :ref="`tree${index}`" :id="`originTree${new Date().getTime()}${index}`" :data="beansStore[item.keyValueBean.infoSetId]" v-model="modelArr[index]"></orgTree>
        ...
        methods: {
            ...
            defaultDataHandler() {
                ...
                this.conditionList.forEach((item,index)=>{
                    ...
                    //可以直接填值的
                    let simpleType = ['1' , '7' , '9' ,'18' ,'19' , '20' , 'originTree'];
            ...
            searchDataHandler(){
                ...
                this.cloneData.forEach((item , index)=>{
                    ...
                    // multiple select
                        ...
                        let bean = this.beansStore[item.keyValueBean.infoSetId];
                        if (bean && this.modelArr[index].length>0){
                            if (this.modelOther[index] && this.modelArr[index].length>0) {
                                obj.value = this.modelOther[index];
                            }else{
                                let arr = [];
                                this.modelArr[index].forEach(id=>{
                                    let b = bean.find(select=>select.id===id);
                                    arr.push({value:id,label : b.name})
                                })
                                obj.value = arr;
                            }
                            search.push(obj)
                    ...
                    // 组织树
                        ...
                        if (this.modelArr[index] && this.modelArr[index].length>0) {
                            obj.value = this.modelArr[index];
                            search.push(obj);
                        }
                    }
                    //大数据单选
            ...
            setValue(value , condition){
                console.log('laji' , value , condition);
            }
        }
    }
