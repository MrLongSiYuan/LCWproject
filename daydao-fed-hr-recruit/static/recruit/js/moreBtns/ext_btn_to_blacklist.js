...
define(function(require, exports, module) {
    ...
    ClassName.prototype = {
        ...
        init : function(options){
            ...
            var _html = '<div id="recruit_black_list" class="recruit_content"><form id="recruit_black_list_form"></form></div>';
            if(aItem.length < 1){
                tools.showMsg.error("请选择需要加入黑名单的数据");
                return;
            }
            t.aFormData = [
                {name:"blackTime",label:"加入日期",type:"date",value:"",isMust:true}
                ,{name:"blackId",label: "黑名单原因",type:"treeSelect",isMust:true,keyValueBean:{infoSetId:"ct_re_05",isDateFilter:false,keyId:"code_id",valueId:"code_name"}}
                ,{name:"blackDesc",label:"备注",type:"textarea",value:"",height:100}
            ...
            t.dialog = dialog({
                ...
                okValue: '保存',
                ok: function () {
                    $('#recruit_black_list').find("form:first").trigger("validate"); //触发表单验证
                    return false;
                },
                cancelValue: '取消',
                cancel: function () {}
            ...
            require.async("commonStaticDirectory/plugins/autoForm/autoForm",function(AutoForm){
                ...
                $form.validator({
                    ...
                    valid:function(form) {
                        ...
                        var oSubmitData = {
                            blacklistTime:$form.find("input[name=blackTime]").val(),
                            blacklistReason:$form.find("input[name=blackId]").val(),
                            blacklistRemark:$form.find("input[name=blackDesc]").val(),
                            resumeIds:aItem
                        };
                        $("body").loading({zIndex:999}) //启用loading遮罩
                        Ajax.ApiTools().addBlacklist({
                            data:oSubmitData
