/**
 * Created by tangguoping on 2017/1/3.
 * 转回收站
 */
define(function(require, exports, module) {
    ...
    ClassName.prototype = {
        ...
        ,init : function(options){
            ...
            t.validatorFields = {};
            if(aItem.length < 1){
                tools.showMsg.error("请选择需要转回收站的数据");
                return;
            };
            for (var i=0; i<aItem.length;i++){
                if(aItem[i].resume_status_id == "已入职"){
                    tools.showMsg.error("已入职状态的简历不能转回收站");
                    return;
                };
            };
            var _html = '<div id="recruit_recycle" class="recruit_content"></div>';
            t.aFormData = [
                {label: "转入日期",name:"recycleDate",type:"date",isMust:true}
                ,{label: "加入黑名单",name:"resumeStatusId",type:"switch",value:"1"}
                ,{label: "黑名单原因",name:"blackReasonId",type:"treeSelect",keyValueBean:{infoSetId:"ct_re_05",isDateFilter:false,keyId:"code_id",valueId:"code_name"}}
            ];
            ...
            t.dialog = dialog({
                ...
                okValue: '保存',
                ok: function () {
                    $('#recruit_recycle').find("form:first").trigger("validate"); //触发表单验证
                    return false;
                },
                cancelValue: '取消',
                cancel: function () {}
        ...
        ,saveData:function () {
            ...
            require.async("commonStaticDirectory/plugins/autoForm/autoForm",function(AutoForm){
                ...
                var $form = $('#recruit_recycle').find("form:first");
                $form.validator({
                    theme: "yellow_right",
                    timely:0, // 保存的时候验证
                    focusCleanup:true,
                    ...
                    valid:function(form) {
                        ...
                        var oSubmitData = {
                            ...
                            ,resumeStatusId:$form.find("input[name=resumeStatusId]").prop("checked")?4:5
                            ,id:_aItem.toString()
                        };
                        if(oSubmitData.resumeStatusId == 4&&oSubmitData.blackReasonId==""){
                            tools.showMsg.error("黑名单原因不能为空！");
                            return;
                        }
                        $("body").loading({zIndex:999}) //启用loading遮罩
                        Ajax.ApiTools().toRecycle({
                            data:oSubmitData
                            ,success:function (data) {
                                tools.showMsg.ok("转回收站成功！");
