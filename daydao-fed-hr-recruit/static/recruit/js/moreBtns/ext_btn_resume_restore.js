...
define(function(require, exports, module) {
    ...
    ClassName.prototype = {
        ...
        ,init : function(options){
            ...
            t.validatorFields = {};
            if(aItem.length < 1){
                tools.showMsg.error("请选择需要还原的数据");
                return;
            };
            for (var i=0; i<aItem.length;i++){
                if(aItem[i].resume_status_id == "黑名单"){
                    tools.showMsg.error("请先解除黑名单再还原简历");
                    return;
                };
            };
            layer.confirm('请确定是要还原选中的简历吗？', {icon: 3, title:'提示'}, function(index){
                //表单验证通过
                var aItem = [];
                for (var i=0; i<t.options.aItem.length;i++){
                    aItem.push(t.options.aItem[i].id);
                }
                Ajax.ajax({
                    type:"GET"
                    ,url:gMain.apiBasePath +"resumeRecycle/restore.do"
                    ,data:{resumeIds:aItem.toString()}
                    ,success:function (data) {
                        if(data.result=="true"){
                            tools.showMsg.ok("还原简历成功！");
