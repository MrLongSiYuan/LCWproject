...
//20M 大于20M的文件需要进行分片传输
    ...
    CosCloud.prototype.cosapi_cgi_url = "https://web.file.myqcloud.com/files/v1/";
    ...
    CosCloud.prototype.dayhrUploadFile = function(success, error, bucketName, remotePath, file, insertOnly){
        var that = this;
        var url = that.cosapi_cgi_url + that.appid + "/" + bucketName + encodeURI(remotePath.cosPath) + "?sign=" + encodeURIComponent(remotePath.sign);
        var formData = new FormData();
        formData.append('op', 'upload');
        formData.append('fileContent', file);
        if(insertOnly>=0){//insertOnly==0 表示允许覆盖文件 1表示不允许 其他值忽略
