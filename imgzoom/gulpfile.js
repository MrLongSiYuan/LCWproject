...
/**
 * ======================================华丽的分割线==========================================
 *   gulp配置  by zhangqi   跟其他项目的区别就是多了个排除 config.js，version.js
 * ======================================华丽的分割线==========================================
 * */
...
var regTask = function (sPrefix) {
    ...
    gulp.task(sPrefix+'_copyfull', function(){
        gulp.src('./static/'+sPrefix+'/**/*.*').pipe(gulp.dest('./dist/static_src/'+sPrefix));
    });
    ...
    gulp.task(sPrefix+'_jsmin', function () {
        gulp.src([
            './static/'+sPrefix+'/**/*.js'
            ,'!./static/'+sPrefix+'/**/*.min.js'
            ,'!./static/'+sPrefix+'/**/zh-cn.js'
            ,'!./static/'+sPrefix+'/version.js'
        ]).pipe(minJs({
            mangle: {except:['require','exports','module','$','jQuery']},//类型：Boolean/Object 默认：true 是否修改变量名
            compress: true//类型：Boolean 默认：true 是否完全压缩
        }))
            .pipe(gulp.dest('./dist/static/'+sPrefix+'/'));
        ...
        gulp.src('./static/'+sPrefix+'/version.js')
            .pipe(replace(/_VERSION_/gi, "gMain" + sDate))
            .pipe(gulp.dest('./dist/static/'+sPrefix));
    });
    ...
    gulp.task(sPrefix+'_imagemin', function () {
        gulp.src('./static/'+sPrefix+'/**/*.{png,jpg,gif,ico}')
            .pipe(imagemin({
    ...
    gulp.task(sPrefix+'_copy', function(){
        gulp.src([
            ...
            ,'./static/'+sPrefix+'/**/*.swf'
            ,'./static/'+sPrefix+'/**/zh-cn.js'
            ,'./static/'+sPrefix+'/config.js'
            ,'./static/'+sPrefix+'/**/*.{eot,svg,ttf,woff}'
        ]).pipe(gulp.dest('./dist/static/'+sPrefix));
    });
...
gulp.task("sonarqube",function(callback){
    sonarqubeScanner({
        ...
        options:{
            "sonar.projectName": "daydao-fed-common-cloudcos",
