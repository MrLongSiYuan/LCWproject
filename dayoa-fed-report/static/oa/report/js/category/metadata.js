...
define(function(require, exports,module) {
    ...
    var formSet = require("js/modules/formSet/formSet.js");
        ...
        ,methods:{
            ...
            initTabel:function(data,queryData){
                ...
                function createTabel(data,queryData){
                    ...
                    var oRules = {}; //验证规则
                    var validatorFields ={}; //待验含有冻结的列
                    var iLastColIndex = -1; //最后证的字段集
                    var isHasFrozen = false; // 是否一个传了冻结的列的名称
                    //头像基础路径
                    if(data.filePrefix){
                    ...
                    //如果是流程设置页面,新表格尾部新增“设置操作”一列(汇报模板列表)
                    if(t.options.infoSetId == "wr_report_templates_list" || t.options.infoSetId == "wf_form_set_list"){
                    ...
                    t.baseJqGrid = $("#" + sTableId).jqGrid({
                        ...
                        , ondblClickRow:function(rowid,iRow,iCol,e){
                            ...
                            if(t.options.infoSetId == "wr_report_templates_list"){ //流程设置页面
                               return false;
                            }else{
                                //编辑数据组装
                                t.doEdit({
                        ...
                        ,gridComplete : function() {
                            ...
                            for ( var i = 0; i < ids.length; i++) {
                                $("#" + sTableId).jqGrid('setRowData', ids[i],{
                                    setting : '<div>' +
                                    '<a href="javascript:void(0)" class="auditInfo" data-rowid="'+ids[i]+'" style="color: #ed6c2b;text-decoration: underline;">基础信息</a> ' +
                                    '<span style="color: #ccc;">|</span> ' +
                                    '<a href="javascript:void(0)" class="formStyle" data-rowid="'+ids[i]+'" style="color: #ed6c2b;text-decoration: underline;">表单信息</a> '
                                    // '<span style="color: #ccc;">|</span> <a href="javascript:void(0)" class="auditStyle" data-rowid="'+ids[i]+'" style="color: #ed6c2b;text-decoration: underline;">表单信息</a></div>'
                                });
                            }
                        }
            ...
            getBtns:function(opts){
                ...
                if(data.functionRange && data.functionRange.length && _.indexOf(data.functionRange,1) != -1){
                    aBtns.push({btnName:"新增",btnfn:function(){
                        ...
                        }else if(t.options.infoSetId == "wr_report_templates_list"){//汇报模板设计
                            new formSet({oMetadata:t,editType:"add"});
                        }else{
                            //特殊插件的表单项处理
            ...
            formateData:function(cellvalue, options, rowObject){
                ...
                //工作汇报表单状态
                if(t.options.infoSetId == "wr_report_templates_list" && options.columnEditParam.name == "status"){
                    return t.setReportState(cellvalue, options, rowObject);
                }
            ...
            EditFormAndAudit:function () {
                var t = this;
                if(t.options.infoSetId == "wr_report_templates_list"){

                    //调用汇报模板信息修改弹窗
                    $("#base_metadata_list").off("click.wf_form_set_list_auditInfo").on("click.wf_form_set_list_auditInfo",".auditInfo",function (e) {
                        var _this = this;
                        new formSet({oMetadata:t,editType:"edit",oFormEditData:t.mmg.row($(_this).data("rowid"))});
                        e.stopPropagation();
                    });


                    //调用表单设计器
                    $("#base_metadata_list").off("click.wr_reform_idport_templates_list_formStyle").on("click.wr_report_templates_list_formStyle",".formStyle",function (e) {
                        var _this = this;
                        $("body").loading({zIndex:9999}); //启用loading

                        var oFormEditData = t.mmg.row($(_this).data("rowid"));
                        oFormEditData.form_name = oFormEditData.name;
                        delete oFormEditData.name;
                        require.async("workflowStaticDirectory/js/modules/formBuild/formBuild.js",function (FormBuild) {
                            new FormBuild({
                                oMetadata:t
                                ,editType:"edit"
                    ...
                    //调用流程设计器
                    // $("#base_metadata_list").off("click.wr_report_templates_list_auditStyle").on("click.wr_report_templates_list_auditStyle",".auditStyle",function (e) {
                    //     var _this = this;
                    //     $("body").loading({zIndex:9999}); //启用loading
            ...
            /*--设置汇报的表单状态--*/
            ,setReportState:function(cellvalue, options, rowObject){
                var t = this;
                var sVal = "";
                if(cellvalue=="1"){
                    sVal='<span style="">'+"启用"+'</span>';
                }else if(cellvalue=="0"){
                    sVal='<span style="color: red;">'+"禁用"+'</span>';
                }else if(cellvalue=="2"){
                    sVal='<span style="color: #f0ad4c;">'+"草稿"+'</span>';
                }
                return sVal;
